...
elseif(isset($_GET['edit']))
{
	...
	{
		$name = $playlist['name'];
		$public = ($playlist['public'] == 'Yes') ? ' checked="checked"' : '';
		$collaborative = ($playlist['collaborative'] == 'Yes') ? ' checked="checked"' : '';
		...
		echo '
			...
			<div class="input_checkbox_div"><div><input type="checkbox" id="edit_playlist_make_public_input"' . $public . '></div><div><label for="edit_playlist_make_public_input">Make public</label></div></div>
			<div class="input_checkbox_div"><div><input type="checkbox" id="edit_playlist_make_collaborative_input"' . $collaborative . '></div><div><label for="edit_playlist_make_collaborative_input">Make collaborative</label></div></div>
...
elseif(isset($_GET['browse']))
{
	...
	else
	{
		...
		else
		{
			...
			$saved = playlist_saved(null, $playlist_uri);

			$details_dialog = array();
			$details_dialog['title'] = $name;
			...
			if($saved) $activity['actions'][] = array('action' => array('Edit', ''), 'keys' => array('actions', 'activity', 'subactivity', 'args'), 'values' => array('change_activity', 'playlists', 'edit', 'uri=' . $playlist_uri));
			...
			$activity['actions'][] = array('action' => array('Share', ''), 'keys' => array('actions', 'title', 'uri'), 'values' => array('share_uri', $name, rawurlencode(uri_to_url($playlist_uri))));
			...
			echo '
				...
				<div id="cover_art_art_div" class="actions_div" data-actions="resize_cover_art" data-imageuri="' . $cover_art_uri . '" data-resized="false" data-width="640" data-height="640" data-uri="' . $playlist_uri . '" style="background-image: url(\'' . $cover_art_uri . '\')" onclick="void(0)"></div>
				<div id="cover_art_information_div" class="shadow_up_black_48_img_div"><div>' . $tracks_count . ' / Playlist by ' . hsc(facebook_user($user)) . '</div></div>
				<div title="Play" id="cover_art_fab_div" class="actions_div play_white_24_img_div" data-actions="show_cover_art_fab_animation play_uri" data-uri="' . $playlist_uri . '" data-highlightclass="light_green_highlight" onclick="void(0)"></div>
				</div>
			';

			echo '<div id="activity_inner_div" data-activitydata="' . base64_encode(json_encode($activity)) . '">';

			if(!$saved) echo '<div id="playlist_description_div"><div id="playlist_description_title_div">Description</div><div id="playlist_description_body_div">' . hsc($description) . '</div><div id="playlist_description_buttons_div"><div class="actions_div" data-actions="import_playlist" data-uri="' . $playlist_uri . '" data-highlightclass="light_grey_highlight" onclick="void(0)">ADD TO MY PLAYLISTS</div></div></div>';
			...
			$class = ($saved) ? 'list_header_below_cover_art_div' : '';
	...
	else
	{
		foreach($playlists as $playlist)
		{
			...
			echo '
				...
				<div class="list_item_main_inner_text_div"><div class="list_item_main_inner_text_upper_div">' . hsc($name) . '</div><div class="list_item_main_inner_text_lower_div">' . hsc(facebook_user($user)) . '</div></div>
				</div>
				</div>
				<div class="list_item_actions_div">
				<div class="list_item_actions_inner_div">
